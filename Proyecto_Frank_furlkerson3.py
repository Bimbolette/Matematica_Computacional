...
class FordFulkersonApp:
    ...
    def editor_interactivo(self):
        ...
        frame_nodos = tk.LabelFrame(frame_izq, text="Configuración del Grafo", bg="#f0f0f0", font=("Arial", 11, "bold"))
        frame_nodos.pack(pady=10, padx=10, fill="x")
        
        tk.Label(frame_nodos, text="Cantidad de nodos:", bg="#f0f0f0", font=("Arial", 10)).pack(pady=(5,2))
        entry_num_nodos = tk.Entry(frame_nodos, font=("Arial", 12), width=10)
        entry_num_nodos.pack(pady=2)
        entry_num_nodos.insert(0, "4")
        
        tk.Label(frame_nodos, text="Nodo Origen (Fuente):", bg="#f0f0f0", font=("Arial", 10, "bold"), fg="#c0392b").pack(pady=(10,2))
        entry_origen_sel = tk.Entry(frame_nodos, font=("Arial", 12), width=10)
        entry_origen_sel.pack(pady=2)
        entry_origen_sel.insert(0, "1")
        
        tk.Label(frame_nodos, text="Nodo Destino (Sumidero):", bg="#f0f0f0", font=("Arial", 10, "bold"), fg="#27ae60").pack(pady=(8,2))
        entry_destino_sel = tk.Entry(frame_nodos, font=("Arial", 12), width=10)
        entry_destino_sel.pack(pady=2)
        entry_destino_sel.insert(0, "4")
        ...
        def agregar_nodos():
            try:
                ...
                origen = int(entry_origen_sel.get())
                destino = int(entry_destino_sel.get())
                
                if origen < 1 or origen > n or destino < 1 or destino > n:
                    messagebox.showerror("Error", f"Los nodos origen y destino deben estar entre 1 y {n}")
                    return
                
                if origen == destino:
                    messagebox.showerror("Error", "El nodo origen y destino no pueden ser el mismo")
                    return
                
                self.num_nodos = n
                self.nodos_grafo = list(range(n))
                self.aristas_grafo = []
                self.nodo_origen = origen - 1
                self.nodo_destino = destino - 1
        ...
        def actualizar_vista():
            ...
            for i, (x, y) in enumerate(posiciones):
                if i == self.nodo_origen:
                    color = "#ffcccb"
                    outline = "#e74c3c"
                elif i == self.nodo_destino:
                    color = "#c8e6c9"
                    outline = "#27ae60"
                else:
                    color = "#b8c5ff"
        ...
        btn_generar = tk.Button(
            frame_botones,
            text="Generar Grafo y Continuar",
            font=("Arial", 14, "bold"),
            bg="#27ae60", fg="white",
            padx=20, pady=10,
    ...
    def ford_fulkerson(self):
        n = len(self.matriz_capacidad)
        fuente = self.nodo_origen
        sumidero = self.nodo_destino
        
        grafo_residual = [fila[:] for fila in self.matriz_capacidad]
        
        self.matriz_flujo = [[0 for _ in range(n)] for _ in range(n)]
        ...
        while self.bfs(grafo_residual, fuente, sumidero, padre):
            ...
            while v != fuente:
                ...
                self.matriz_flujo[u][v] += flujo_camino
                v = padre[v]
    ...
    def mostrar_grafo_con_corte(self):
        ...
        info = tk.Label(
            frame_principal,
            text="Las aristas ROJAS representan el corte mínimo. Formato: Capacidad/Flujo",
        ...
        for i in range(n):
            for j in range(n):
                if self.matriz_capacidad[i][j] > 0:
                    ...
                    capacidad = self.matriz_capacidad[i][j]
                    flujo = self.matriz_flujo[i][j]
                    etiqueta = f"{capacidad}/{flujo}"
                    
                    bg_color = "#ffe6e6" if es_corte else "white"
                    canvas.create_rectangle(mx-25, my-12, mx+25, my+12, fill=bg_color, outline=color if es_corte else "")
                    canvas.create_text(
                        mx, my,
                        text=etiqueta,
                        font=("Arial", 10, "bold"),
